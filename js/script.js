...
function agregarEventos(array) {
  //Recibe el array de las pelis filtradas para aplicar pasarlo como argumento a la funcion que le agrega a las pelis en la lista
...
function offcanvasMostrarDatos(array, id) {
  let generos = document.getElementById("contenido-generos");
  let contenidoGeneros = ``;

  for (let i of array[id].genres) {
    contenidoGeneros += `${i.name} - `;
  }
  let contenidoFinal = contenidoGeneros.slice(0, contenidoGeneros.length - 2);

  generos.innerHTML = contenidoFinal;
